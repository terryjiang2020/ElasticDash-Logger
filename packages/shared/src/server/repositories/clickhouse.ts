    ...
    async (span) => {
      ...
      await Promise.all(
        opts.records.map(async (record) => {
          ...
          const bucketPath = `${env.ELASTICDASH_S3_EVENT_UPLOAD_PREFIX}${record.project_id}/${getClickhouseEntityType(eventType)}/${record.id}/${eventId}.json`;

          if (env.ELASTICDASH_ENABLE_BLOB_STORAGE_FILE_LOG === "true") {
  ...
  return await instrumentAsync(
    ...
    async (span) => {
      ...
      return await backOff(
        ...
        {
          numOfAttempts: env.ELASTICDASH_CLICKHOUSE_QUERY_MAX_ATTEMPTS,
          retry: (error: Error, attemptNumber: number) => {
            const shouldRetry = isRetryableError(error);
            if (shouldRetry) {
              logger.warn(
                `ClickHouse query failed with retryable error (attempt ${attemptNumber}/${env.ELASTICDASH_CLICKHOUSE_QUERY_MAX_ATTEMPTS}): ${error.message}`,
                {
                  error: error.message,
                  attemptNumber,
                  tags: opts.tags,
